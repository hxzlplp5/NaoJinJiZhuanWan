...
def get_tianhang(config):
    # 获取早安心语，晚安心语，健康小提示，土味情话，生活小窍门， 彩虹屁，毒鸡汤，舔狗日记，小段子，朋友圈文案，文化谚语 ,雷人笑话,谜语大全，脑筋急转弯,失恋分手句子，藏头诗,故事大全
    data = ["zaoan", "wanan", "healthtip", "saylove", "qiaomen", "caihongpi", "dujitang", "tiangou", "mnpara",
            "pyqwenan", "proverb","joke","riddle","naowan","hsjz","cangtoushi","story"]
    data_list = {}
    for i in data:
        try:
            key = config["tian_api"]
            ...
            response = get(url, headers=headers).json()
            if response["code"] == 200:
                if i != "proverb" and i !="riddle" and i!="naowan" and i!="story":
